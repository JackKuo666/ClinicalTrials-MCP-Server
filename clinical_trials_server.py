...
# Set output character limit threshold
MAX_OUTPUT_CHARS = 5000

# Create an MCP server
mcp = FastMCP("ClinicalTrials-MCP-Server")
...
def load_corona_fields():
    """Load the corona fields data from CSV"""
    return load_csv_file("corona_fields.csv")
...
# Resources
@mcp.resource("clinicaltrials://corona_fields")
def get_corona_fields() -> str:
...
def get_studies_by_condition(condition: str) -> str:
    ...
    df = load_corona_fields()
    if df is not None and "Conditions" in df.columns:
        studies = df[df["Conditions"].str.contains(condition, case=False, na=False)]
        if not studies.empty:
            return format_limited_output(studies)
